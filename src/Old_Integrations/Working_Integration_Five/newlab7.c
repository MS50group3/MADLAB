...
void draw_room(SDL_Surface *background, SDL_Surface *sprite, SDL_Texture *backtex, SDL_Texture *spritetex, roomGrid *rg);
...
void draw(roomGrid *rg, progress *pz, Mix_Chunk *mus, char *instructions_list[NUM_INSTRUCTIONS])
{
    ...
    initialise_roomgrid_components(rg, pz);
    ...
    while (rg -> gamerunning)
    {
        ...
        movement(rg, pz, instructions_list);     

        collision_detection(rg);

        SDL_RenderClear(rg -> renderer);

        draw_room(background, sprite, backtex, spritetex, rg);
...
void draw_room(SDL_Surface *background, SDL_Surface *sprite, SDL_Texture *backtex, SDL_Texture *spritetex, roomGrid *rg)
{
    for (int i = 0; i <= ROOM_Y; i++){
        for (int j = 0; j < ROOM_X; j++){
            ...
            //You also free up 0!
            rcsrc_set(41, 0, 32, 29, backtex, rg);
            
            if (rg -> arr[i][j] == 1){
                rcsrc_set(71, 81, 45, 34, backtex, rg);
            }

            if (rg -> arr[i][j] == 2){
                rcsrc_set(73, 0, 32, 29, backtex, rg);
            }

            if (rg -> arr[i][j] == 3){
                rcsrc_set(121, 56, 76, 59, backtex, rg);
            }

            if (rg -> arr[i][j] == 4){
                rcsrc_set(207, 70, 61, 41, backtex, rg);
            }

            if (rg -> arr[i][j] == 5){
                rcsrc_set(279, 8, 55, 127, backtex, rg);
            }

            if (rg -> arr[i][j] == 6){
                rcsrc_set(141, 0, 32, 32, backtex, rg);
            }
