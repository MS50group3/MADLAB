...
void draw(roomGrid *room_grid, progress *puzzle, Mix_Chunk *mus, char *instructions_list[NUM_INSTRUCTIONS]);
void run_main_game(roomGrid *room_grid, progress *puzzle);
void freeroom_array(roomGrid *room_grid);
void possible(roomGrid *room_grid, progress *puzzle);
int action(roomGrid *room_grid, progress *puzzle, char *instructions_list[NUM_INSTRUCTIONS]);
void interactProbe(roomGrid *room_grid, progress *puzzle, char *instructions_list[NUM_INSTRUCTIONS]);
...
void movement(roomGrid *room_grid, progress *puzzle, char *instructions_list[NUM_INSTRUCTIONS]);
...
int main(int aroom_gridc, char *aroom_gridv[])
{
    ...
    run_main_game(room_grid, puzzle);
...
void run_main_game(roomGrid *room_grid, progress *puzzle)
{
    ...
    draw(room_grid, puzzle, mus, instructions_list);
...
void draw(roomGrid *room_grid, progress *puzzle, Mix_Chunk *mus, char *instructions_list[NUM_INSTRUCTIONS])
{
    ...
    while (room_grid -> gamerunning)
    {
        ...
        movement(room_grid, puzzle, instructions_list);     
        ...
        switch (event.type) 
        {
            ...
            switch (event.key.keysym.sym)
            {
                ...
                case SDLK_DOWN:
                    ...
                    break;
                case SDLK_SPACE:
                    interactProbe(room_grid, puzzle, instructions_list);
                    break;
            }
            break;
        }
...
void possible(roomGrid *room_grid, progress *puzzle)
{
    ...
    {       
        ...
        case(up):       if ((room_grid -> room_array[room_grid -> top_y_coord-1][room_grid -> left_x_coord]) == (room_grid -> room_array[room_grid -> top_y_coord-1][room_grid -> right_x_coord])){
                            if (room_grid -> room_array[room_grid -> top_y_coord - 1][room_grid -> left_x_coord] == 0){
...
void move(roomGrid *room_grid, progress *puzzle)
{
    switch(room_grid -> direction)
    {       
        case(left):     (room_grid -> rcObj.x == 0) ? (room_grid -> rcObj.x = TILE_SIZE): (room_grid -> rcObj.x = 0);
                        room_grid -> rcSprite.x -= MOVEMENT_INCREMENT;
                        break;

        case(down):     (room_grid -> rcObj.x == 0) ? (room_grid -> rcObj.x = TILE_SIZE): (room_grid -> rcObj.x = 0);
                        room_grid -> rcSprite.y += MOVEMENT_INCREMENT;                  
                        break;

        case(right):    (room_grid -> rcObj.x == 0) ? (room_grid -> rcObj.x = TILE_SIZE): (room_grid -> rcObj.x = 0);
                        room_grid -> rcSprite.x += MOVEMENT_INCREMENT;
                        break;

        case(up):       (room_grid -> rcObj.x == 0) ? (room_grid -> rcObj.x = TILE_SIZE): (room_grid -> rcObj.x = 0);
                        room_grid -> rcSprite.y -= MOVEMENT_INCREMENT;
                        break;
...
void interactProbe(roomGrid *room_grid, progress *puzzle, char *instructions_list[NUM_INSTRUCTIONS])
{
    ...
    {       
        case(left):     room_grid -> probe = room_grid -> room_array[room_grid -> y_sprite_centre][room_grid -> x_sprite_centre - 1];
                        if (action(room_grid, puzzle, instructions_list)){
        ...
        case(up):       room_grid -> probe = room_grid -> room_array[room_grid -> y_sprite_centre - 1][room_grid -> x_sprite_centre];
                        if (action(room_grid, puzzle, instructions_list)){
...
void endFrame(roomGrid *room_grid){
    SDL_UpdateWindowSurface(room_grid->window);
}
