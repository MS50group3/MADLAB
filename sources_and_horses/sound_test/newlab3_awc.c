...
void draw(SDL_Renderer *renderer, SDL_Window *window, roomGrid *rg, progress *pz, Mix_Music *mus);
...
void run(roomGrid *rg, progress *pz)
{
  ...
  {
    window = SDL_CreateWindow ("MADLAB v. 0.1",             //window name
  ...
  // Inilialise the sound 
  if(!(Mix_Init(MIX_INIT_OGG) & MIX_INIT_OGG))
  { 
    printf("Mix_Init could not initialise! You are in trouble. Error\n");
    exit(1);
  }
  // Setup audio mode
...
- this needs to get put into a function*/
    ...
    rg -> rcSrc1.w =  61;
    rg -> rcSrc1.h =  41;
    SDL_RenderCopy(renderer, backtex, &rg -> rcSrc1, &rg ->rcObj);
