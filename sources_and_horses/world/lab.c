...
void possible(roomGrid *rg)
{
	...
	switch(rg -> direction)
	{		
		case(left):		if ((rg -> arr[rg -> ya][rg -> xa - 1] == path) && (rg -> arr[rg -> yb][rg -> xa - 1] == path))
						{
		...
		case(down):		if((rg -> arr[rg -> yb + 1][rg -> xa] == path) && (rg -> arr[rg -> yb + 1][rg -> xb] == path))
						{
		...
		case(right):	if((rg -> arr[rg -> ya][rg -> xb + 1] == path) && (rg -> arr[rg -> yb][rg -> xb + 1] == path))
						{
		...
		case(up):		if((rg -> arr[rg -> ya - 1][rg -> xa] == path) && (rg -> arr[rg -> ya - 1][rg -> xb] == path))
						{
							// rg -> probe = rg -> arr[rg -> ya][rg -> xa - 1];
							// if(walk(rg))
							// {
							// 		rg -> rcSprite.y -= 8;
							// }
